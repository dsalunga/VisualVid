...
namespace DES.Web.CMS
{
	...
	public partial class QueryAnalyzer : System.Web.UI.Page
	{
		...
		protected void cmdExecute_Click(object sender, System.EventArgs e)
		{
            ...
            {
                if (sQS == string.Empty)
                    sConnString = SqlHelper.CONN_STRING;
                else
            ...
            if (chkUpload.Checked)
            {
                ...
                string sFolder = Server.MapPath("Files");
                ...
                else
                {
                    ...
                    ds_import.ReadXml(sFilePath);
                    /*
                    if (chkSchema.Checked)
                    {
                        ds_import.ReadXml(sFilePath, XmlReadMode.ReadSchema);
                    }
                    else
                    {
                        ds_import.ReadXml(sFilePath, XmlReadMode.IgnoreSchema);
                    }
                     */
                    da.Update(ds_import.Tables[0]);
                }
            }
            else
            {
        ...
        private void DisplayDataGrid(DataSet ds)
        {
            ...
            if (bDownload)
            {
                try
                {
                    string sFolder = Server.MapPath("Temp");
